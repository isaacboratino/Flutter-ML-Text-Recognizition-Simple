...
import 'package:firebase_ml_vision/firebase_ml_vision.dart' as firebaseVision;
...
class _DetailState extends State<DetailWidget> {
  ...
  void analyzeLabels() async {    
    ...
    try {
      ...
      if (widget._scannerType == TEXT_SCANNER) {
        ...
        final firebaseVision.FirebaseVisionImage visionImage = firebaseVision.FirebaseVisionImage.fromFile(widget._file);
        final firebaseVision.TextRecognizer textRecognizer = firebaseVision.FirebaseVision.instance.textRecognizer();
        final firebaseVision.VisionText visionText = await textRecognizer.processImage(visionImage);       

        final String logs = visionText.text;

        print('visionText: $logs');
        debugPrint('visionText: $logs');
        log('visionText: $logs');        
        
        currentLabels = await textDetector.detectFromPath(widget._file.path);
        //currentLabels = visionText.blocks;
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          centerTitle: true,
          title: Text(widget._scannerType == TEXT_SCANNER ? 'Text rec' : 'Barcode Scanner'),
        ),
